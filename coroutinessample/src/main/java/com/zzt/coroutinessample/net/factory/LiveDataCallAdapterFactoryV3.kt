...
import com.zzt.coroutinessample.modle.HttpResponseV3
import retrofit2.*
                    ...
                    if (flag.compareAndSet(false, true)) {
                        call!!.enqueue(object : Callback<R> {
                            override fun onFailure(call: Call<R>?, t: Throwable?) {
                                val d = Log.d("数据请求失败", "" + call?.request()?.body.toString())
                                val value = HttpResponseV3<Any>(false, "解析类型失败", "-1") as R
                                postValue(value)
                            }

                            override fun onResponse(call: Call<R>?, response: Response<R>?) {
                                val d = Log.d("数据请求成功", "" + response?.body()?.toString())
